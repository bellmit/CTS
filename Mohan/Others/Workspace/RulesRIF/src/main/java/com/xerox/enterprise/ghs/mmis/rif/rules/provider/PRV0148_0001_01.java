...
/**
 * POJO for the rule "PRV0148.0001.01"
 * @author $authorName
 * @source $ruleDescriptor.getRuleExtractClassName()
 */
public class PRV0148_0001_01 extends Rule {
	
    /* Static logger to log a message */
    private static final Logger LOG = Logger.getLogger(PRV0148_0001_01.class);
    ...
    public static RulesResult execute(RulesContext ctx, String ruleId) throws RIFPOJORulesException {
        ...
        if (ctx != null) {
            ...
            systemList2 = (List) inputObjs.get(1);
            if (systemList2 == null) {
                LOG.error("Input Object systemList2 is  null during intialization for the rule " + ruleId);
                throw new RIFPOJORulesException("Unable to initialize the required rules objects for rule " + ruleId);
            }
        ...
        // EXTRACT_START PRV0148.0001.01
        boolean flag2 = getClassSystemValue(systemList2, prvNumber);

        if (flag2) {
            providerDO.setTypeClassificationCode(ProviderDataConstants.PRV_ENR_VALUE_TWO);
        }
        // EXTRACT_END
